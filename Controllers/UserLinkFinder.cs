...
namespace SixDegrees.Controllers
{
    class UserLinkFinder : DegreeLinkFinder<UserResult>
    {
        ...
        protected override void EnsureLinksToNext<TPath>(Dictionary<string, ICollection<UserResult>> cachedConnections, string key, ConnectionInfo<TPath>.Node node)
        {
            if (!cachedConnections[key].Contains(node.Value as UserResult))
                cachedConnections[key].Add(node.Value as UserResult);
        }

        protected override async Task<IActionResult> FindConnections(UserResult query, bool allowAPICalls) =>
